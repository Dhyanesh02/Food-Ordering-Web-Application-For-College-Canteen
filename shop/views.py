...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            allProds.append([prod, range(1, nSlides), nSlides])
    dhyanesh = {'allProds': allProds, "msg": ""}
    if len(allProds) == 0 or len(query) < 3:
        dhyanesh = {'msg': "No item available. Please make sure to enter relevant search query"}
    return render(request, 'shop/search.html', dhyanesh)
...
def checkout(request):
    if request.method == "POST":
        ...
        email = request.POST.get('email', '')
        phone= request.POST.get('phone','')
        serve_time = request.POST.get('serve_time','')
        order = Orders(items_json=items_json, userId=user_id, name=name, email=email, phone=phone,serve_time=serve_time, amount=amount)
        order.save()
        ...
        if 'onlinePay' in request.POST:
            ...
            dhyanesh_dict['CHECKSUMHASH'] = Checksum.generate_checksum(dhyanesh_dict, MERCHANT_KEY)
            return render(request, 'shop/paytm.html', {'dhyanesh_dict': dhyanesh_dict})
        elif 'cashOnDelivery' in request.POST:
            return render(request, 'shop/checkout.html', {'thank': thank, 'id': id})
    # if request.method=='POST':
    #     amount= str(amount)
    #     order_currency='INR'
    #     client = razorpay.Client(auth=("rzp_test_EOs7phZ9i1sEp0", "WgWJwonoWylgM11k943s1kvB"))
